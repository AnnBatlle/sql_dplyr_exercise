...
match_db %>% 
  ...
  select(home_team=home_team_api_id, home_team_goals=home_team_goal, country_id) %>% 
  left_join(
    {country_db %>% 
  ...
  group_by(home_team) %>% 
  summarise(goals=mean(home_team_goals, na.rm=TRUE)) %>% 
  arrange(desc(goals)) %>% 
  head(1) %>% 
  left_join(
    {teams %>% 
      select(home_team=team_api_id, name=team_long_name)},
    by='home_team') %>% 
  select(name)
```
...
```{r}
match_db %>% 
  select(home_team=home_team_api_id, home_team_goals=home_team_goal, country_id, season) %>% 
  group_by(home_team, country_id, season) %>% 
  summarise(goals=sum(home_team_goals, na.rm=TRUE)) %>% 
  arrange(desc(goals)) %>% 
  head(1) %>% 
  left_join(
    {teams %>% 
  ...
  left_join(
    {country_db %>% 
      select(country_id=id, country_name=name)},
    by='country_id') %>% 
  select(team_id=home_team, team=name, country_id, country=country_name, season, goals)
```
